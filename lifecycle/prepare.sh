...
function install_custom_components() {
  ...
  current_hash=$(_sha256 "${manifest}") \
    || { echo "[custom_components] ERROR: failed to hash manifest"; return 2; }
  ...
  local tmp
  tmp=$(mktemp -d) \
    || { echo "[custom_components] ERROR: failed to create temporary directory"; return 2; }
  ...
  rm -rf "${DATA_PATH}/custom_components" \
    || { echo "[custom_components] ERROR: failed to remove ${DATA_PATH}/custom_components"; return 2; }
  mkdir -p "${DATA_PATH}/custom_components" \
    || { echo "[custom_components] ERROR: failed to create ${DATA_PATH}/custom_components"; return 2; }
  cp -rf "${tmp}/." "${DATA_PATH}/custom_components/" \
    || { echo "[custom_components] ERROR: cp to ${DATA_PATH}/custom_components/ failed"; return 2; }

  _save_hash "${STATE_PATH}" "custom_components" "${current_hash}" \
    || { echo "[custom_components] ERROR: failed to save state hash"; return 2; }
...
function install_www_components() {
  ...
  local current_hash
  current_hash=$(_sha256 "${manifest}") \
    || { echo "[www_components] ERROR: failed to hash manifest"; return 2; }
  ...
  local tmp
  tmp=$(mktemp -d) \
    || { echo "[www_components] ERROR: failed to create temporary directory"; return 2; }
  trap 'rm -rf "${tmp}"; trap - RETURN' RETURN
  ...
  while read -r component; do
    if [[ -n "${component//[[:space:]]/}" && "${component:0:1}" != "#" ]]; then
      ...
      mkdir -p "${tmp}/${data[3]}" \
        || { echo "[www_components] ERROR: failed to create ${tmp}/${data[3]}"; return 2; }
      local dl_err
      if [ "${data[4]:-}" = "file" ]; then
        ...
        dl_err=$({ wget -qO- "${data[0]}/${data[1]}/${data[2]}" | tar xz -C "${tmp}/${data[3]}"; } 2>&1) \
          || { echo "[www_components] ERROR: download/extract failed for ${data[2]}: ${dl_err}"; return 2; }
      fi
  ...
  rm -rf "${DATA_PATH}/www" \
    || { echo "[www_components] ERROR: failed to remove ${DATA_PATH}/www"; return 2; }
  mkdir -p "${DATA_PATH}/www" \
    || { echo "[www_components] ERROR: failed to create ${DATA_PATH}/www"; return 2; }
  cp -rf "${tmp}/." "${DATA_PATH}/www/" \
    || { echo "[www_components] ERROR: cp to ${DATA_PATH}/www/ failed"; return 2; }

  _save_hash "${STATE_PATH}" "www_components" "${current_hash}" \
    || { echo "[www_components] ERROR: failed to save state hash"; return 2; }
